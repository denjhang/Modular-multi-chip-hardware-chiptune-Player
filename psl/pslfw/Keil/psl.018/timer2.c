	...
	for(i=0; i<len; ){
		switch(OUT2BUF[i]){
			case 0x00:
				...
				TR2 = 0;
				ResetDevice();
				//timer, 1sync=1/100Hz, 0x10000-((24M/4)/100Hz)=0x15a0
				SetTimer2(0x15, 0xa0);
				EA = 1;
				i++;
				break;
...
void TD_Poll(void)
{
	...
	//pipe1
	//  data command
	if(!(EPIO[OUT4BUF_ID].cntrl&bmEPBUSY)){
		//
		len = g_wLength+OUT4BC;
		if(len>LENGTH){
			//buffer full
		} else {
			//
			AUTOPTRH = MSB(OUT4BUF);
			AUTOPTRL = LSB(OUT4BUF);
			ptr = g_byWptr+OUT4BC;
			if(ptr>LENGTH){
				//
				...
				//
				BufCopy(g_byBuf+g_byWptr, OUT4BC);
			}
			//
			OUT4BC = 0;
			g_wLength = len;
